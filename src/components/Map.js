...
export const Map = () => {
  ...
  const last5minData = _.map(polyTrashData.slice(0, 299), d => {
    return (THRASH_SIZE - (d - 5)) / THRASH_SIZE;
  });

  data.datasets[0].data = last5minData;

  const currentPercentage = Math.min(
    Math.max(0, Math.round(last5minData[0] * 100)),
    100
  );
  useEffect(() => {
    ...
    if (currentPercentage > treshHold) {
      coordinatesString += POLY_LONG + ',';
      coordinatesString += POLY_LAT + ';';
    }

    for (let trash of montrealTrashData
      .filter(trash => trash.fullness > treshHold)
      .slice(0, 23)) {
    ...
    fetch(
      `https://api.mapbox.com/directions/v5/mapbox/walking/${coordinatesString}?geometries=geojson&access_token=${ACCESS_TOKEN}`,
      {
        method: 'GET',
        headers: {
      ...
      res.json().then(resJson => {
        ...
        const routes = resJson.routes && resJson.routes[0].geometry.coordinates;
        if (routes) {
          setDirectionData(routes);
        }
      });
    });
  }, [montrealTrashData, polyTrashData]);
  ...
  return (
    <>
      ...
      >
        ...
        <Layer type="line">
          <Feature coordinates={directionData} />
        </Layer>
      </ReactMapboxGlMap>
    </>
  );
